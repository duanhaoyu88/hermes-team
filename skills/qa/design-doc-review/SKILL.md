...
### Step 5b.ii — YAML/Config schema review (skill-map, config maps)

When reviewing a **YAML schema definition** (like skill-map.yaml, role config, or any declarative mapping):

1. **Tier classification**: Tag each field as Tier 1 (必填), Tier 2 (强建议), Tier 3 (可选). Check:
   - Tier 1 fields support the core function of the schema (if missing, the schema is broken)
   - Tier 2 fields support auditability and maintainability (if missing, the schema works but is hard to maintain)
   - Tier 3 is optional — don't block on missing Tier 3, but flag if it causes a known gap

2. **Semantic consistency audit** — high-value check for declarative schemas with enum + free-text fields:
   ```
   | Field         | Value | Text field | Conflict? |
   |---------------|-------|------------|-----------|
   | load: auto    | auto  | "审查时加载"  | ⚠️ auto means startup load, text describes on-demand |
   | load: manual  | manual| "提交 PR 时" | ✅ manual + conditional text are consistent |
   | load: auto    | auto  | "启动时加载"  | ✅ consistent |
   ```
   Pattern: when an enum field (auto/manual/never) has a companion free-text field (when/description), they must be semantically aligned. auto + "审查时加载" means "loaded at startup but the text says loaded on demand" — this is a semantic bug.

3. **Evidence-based allocation challenge**: Cross-reference declared load policy with actual usage data:
   - Run `skills_list()` and track which skills were actually loaded in the session
   - Compare: `allocated_auto = N` vs `actually_used = M`
   - If M/N < 50% for multiple sessions, challenge the auto allocation
   - Challenge format: quantitative. Don't say "feels like too many", say "3 auto skills, 1 actually used in this session (33%). 2 skills never loaded. Recommend demoting to manual."

4. **Never-classification audit**: Items marked `load: never` in a published config:
   - If a skill exists in the schema with `never`, ask: "Why is it here? If it should never be loaded, it shouldn't be in the map at all."
   - `never` in a published map is a design debt — it clutters the map and confuses reviewers
   - Correction: remove from published map entirely, or move to an archive section

5. **Cross-file referential integrity**: For a multi-file config (skill-map.yaml + skill-index.yaml):
   - Every name in skill-index.yaml must exist in skill-map.yaml
   - Every `knowledge_ref` must point to a real Obsidian path or be omitted
   - `roles` in skill-map must match the role names in skill-index sections

### Step 5b.iii — Phase plan review (multi-phase implementation plans)

When reviewing a multi-phase implementation plan (Phase 0 → Phase N):

1. **Phase dependency audit**: Check each phase's prerequisites against previous phase outputs:
   ```
   | Phase | Output | Needed By | Gap? |
   |-------|--------|-----------|------|
   | Phase 0: skill-inventory.md | Phase 1: skill-map draft | ✅ |
   | Phase 1: skill-map draft    | Phase 2: loading design | ⚠️ map schema not yet decided |
   ```
   If Phase 1 output is a draft but Phase 2 depends on specific schema fields, add a "Schema decision node" between them.

2. **Materials-request audit for review phases**: When a phase is labeled "review" (e.g., Phase 4 — 审查), define upfront what materials the reviewer needs:
   ```
   Phase 4 review requires:
   - Output file(s) from Phase 3
   - Input file(s) from Phase 0-2 (for cross-reference)
   - Process trace (dry-run output, not just final result)
   ```
   If any material is not produced by previous phases, flag it as a gap.

3. **Splitting heuristic**: A phase that produces 3+ independent deliverables should be split. e.g.:
   - Phase 3: "create map + create script + archive repo" → Phase 3a, 3b, 3c
   - Rationale: map review can't start while script is still being written, and archive shouldn't depend on either.

### Step 5b.iv — Self-audit driven design challenge

Use actual runtime data from your own session to challenge design allocations quantitatively:

1. **Session capture**: At start of session, run `skills_list()` to record available skills
2. **Usage tracking**: Record which skills you actually loaded via `skill_view()` or used
3. **Allocation challenge**: Compare design's proposed allocation against your measured usage:
   ```yaml
   Proposed: qa → auto: [A, B, C]
   Actual:   qa → auto: [A only]  (B and C never loaded)
   Gap: 33% utilization → challenge
   ```
4. **Formulate challenge**: Present as data, not opinion. Include the utilization percentage and a specific recommendation (which skills to demote).

This pattern is uniquely available to the QA Agent because you're both the reviewer and the subject of the allocation — use it.
