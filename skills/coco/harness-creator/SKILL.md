...
description: >-
  ...
  Auto-applied to every code project. Creates AGENTS.md, feature_list.json,
  progress.md, init.sh, session-handoff.md.
when_to_use: >-
  ...
  **ALWAYS applied to new code projects unless user explicitly opts out.**
license: MIT
---
...
### Phase 4: Draft & Review (MANDATORY STOP)

**CRITICAL: Do NOT write files yet.**
1. **Draft**: Generate the full content for `AGENTS.md`, `feature_list.json`, `init.sh`, etc.
2. **Review**: Show the drafts to the user. Explicitly ask: "Is this logic and content qualified?"
3. **Wait**: Stop generation. Do not proceed until the user approves.

### Phase 5: Implementation

**Execute ONLY after user approves the draft content:**
1. **Create**: Write the files to disk with approved content.
2. **Verify**: Run `init.sh` to ensure environment is healthy.
...
2. **With Harness**: Run the same task with the harness in place
...
## 🌐 Long-Term Execution Protocol (GitHub + Wiki + Harness Trinity)

**Applicable to all long-running coding projects.** This section defines the mandatory workflow for executing features over weeks/months.

### 1. Repository Structure (Code vs Docs Separation)

```
repo-root/
├── src/ (or firmware/, etc.)    # Source code → MUST be in Git
├── AGENTS.md                    # Harness constitution → MUST be in Git
├── feature_list.json            # State tracker → MUST be in Git
├── ARCHITECTURE.md              # Project-scoped design → MUST be in Git
└── docs/                        # Project-specific docs only
```

**⚠️ Rule**: Do NOT commit the entire external Wiki (e.g. `~/Obsidian/`) to the project Git.
- Wiki is the **external brain** (generic chip specs, domain knowledge).
- Only `ARCHITECTURE.md` and project-tightly-coupled docs go into Git.

### 2. The Feature Loop (Branch-per-Feature)

**Never write directly on `main` for long-term features.**

1. **Branch Isolation**:
   - Task `feat-02` → Create branch `feat/02-screen-init`.
   - Agent **must** check out the correct branch before writing any code.

2. **Knowledge-Sync Commit**:
   - If code has hardware params (e.g. `0x77` I2C addr), update `ARCHITECTURE.md` **in the same commit**.
   - Update `~/Obsidian/entities/` Wiki page **in parallel**.
   - **Commit Message Template**:
     ```
     feat: [feat-XX] Short description

     - Set <param> to <value>, confirmed in <source>.
     - Update ARCHITECTURE.md <section>.
     - Sync Obsidian entities/<chip>.md with BSP implementation.
     ```

3. **PR as Verification Gate**:
   - Push branch and open **Pull Request**.
   - User reviews Diff via GitHub App on phone.
   - **User Merge = Acceptance**. Agent does not merge itself.

### 3. Pre-commit Safety Hooks

Add checks (via `init.sh` or git hooks) before every commit:
- **Block `build/`, `cache/`, large binaries**.
- **Validate `feature_list.json`** (JSON syntax check).
- **Magic Number Scan**: Warn if hardcoded literals exist without macro definition.
...
## 🔴 强制规则 (Mandatory Enforcement)
**每次用户启动新项目或要求编写/重构核心模块时，必须自动创建 5 子系统 Harness。**
- 不需要询问 "要不要建？"。直接生成草案等拍板（Draft & Review）。
- 默认配置：AGENTS.md + feature_list.json + progress.md + init.sh。
- 除非用户明确说"不要 Harness"，否则视为必须交付件。
